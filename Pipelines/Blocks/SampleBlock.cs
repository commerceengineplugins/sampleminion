...
namespace Plugin.Sample.Minion
{
    ...
    public class SampleBlock : PipelineBlock<SampleArgument, SampleEntity, CommercePipelineExecutionContext>
    {
        ...
        /// </returns>
        public async override Task<SampleEntity> Run(SampleArgument arg, CommercePipelineExecutionContext context)
        {
            Condition.Requires(arg).IsNotNull("The argument can not be null");
            var result = await this._pipeline.Run(arg, context);
            return result;
        }
    }
}
